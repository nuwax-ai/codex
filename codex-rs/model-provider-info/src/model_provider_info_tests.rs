...
fn test_deserialize_chat_wire_api() {
    // This fork re-enables `wire_api = "chat"` (Chat Completions protocol via
    // codex-rust-genai-bridge), so it must deserialize successfully instead of
    // being rejected like upstream.
    let provider_toml = r#"
